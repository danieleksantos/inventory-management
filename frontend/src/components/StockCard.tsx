  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-inventory-100 shadow-sm hover:shadow-xl transition-all duration-300 group">
      <div className="flex justify-between items-start mb-4">
        ...
        <span className={`text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded-full text-white ${getStatusColor(material.stockQuantity)}`}>
          {material.stockQuantity <= 20 ? 'Critical' : material.stockQuantity <= 50 ? 'Warning' : 'Stable'}
        </span>
      </div>

      <div>
        <h3 className="text-sm font-bold text-inventory-500 uppercase tracking-tight">{material.name}</h3>
        <div className="flex items-baseline gap-1">
          <span className="text-4xl font-black text-inventory-800">{material.stockQuantity}</span>
