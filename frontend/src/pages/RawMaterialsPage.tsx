...
export function RawMaterialsPage() {
  ...
  return (
    <div className="animate-in fade-in duration-500 pb-10">
      <Header title="Gestão de" highlight="Matéria-Prima" subtitle="Controle de estoque industrial" />
      ...
      <div className="relative">
        {loading && <div className="absolute inset-0 bg-white/60 z-10 flex items-center justify-center rounded-4xl"><Loader2 className="animate-spin text-accent-primary" /></div>}
        
        <div className="grid grid-cols-1 gap-6 md:hidden">
          {items.map(item => (
            <div key={item.id} className="bg-white rounded-4xl border border-inventory-100 shadow-sm overflow-hidden flex flex-col">
              <div className="p-8 flex flex-col gap-5">
                <div className="space-y-1">
                  <p className="text-[9px] font-black text-inventory-300 uppercase tracking-[0.2em]">Insumo</p>
                ...
                <div className="flex items-end justify-between border-t border-inventory-50 pt-4">
                  <div className="space-y-1">
                    <p className="text-[9px] font-black text-inventory-300 uppercase tracking-[0.2em]">Quantidade</p>
                    <p className="text-xl font-black text-accent-primary italic tracking-tighter">
                      {item.stockQuantity} <span className="text-[10px] text-inventory-400 not-italic ml-1 uppercase">unid.</span>
                    </p>
                  </div>

                  <Button 
                    variant="secondary" icon={Edit3} onClick={() => handleOpenEdit(item)}
        ...
        <div className="hidden md:block bg-white rounded-4xl border border-inventory-100 shadow-sm overflow-hidden">
          <table className="w-full text-left font-bold">
            ...
            <tbody className="divide-y divide-inventory-50 uppercase text-sm">
              {items.map(item => (
                <tr key={item.id} className="hover:bg-inventory-50/10 transition-colors">
                  <td className="px-8 py-4 text-inventory-800 font-black">{item.name}</td>
                  <td className="px-8 py-4 text-center text-inventory-600">
                    {item.stockQuantity} <span className="text-[10px] text-inventory-400 ml-1">unid.</span>
                  </td>
      ...
      <RawMaterialModal 
        isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleSave} onDelete={handleDelete}
        editingItem={editingItem} formData={formData} setFormData={setFormData} isSubmitting={isSubmitting}
      />
    </div>
  );
}